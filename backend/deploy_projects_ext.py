...
class FixRequest(BaseModel):
    """A natural-language description of the problem to fix. Usually composed by
    the Fix problem button from a failed deploy / health check (error + likely
    cause + suggested fixes)."""
    instruction: str = Field(min_length=2, max_length=8_000)


@ops_router.post('/{project_id}/fix')
async def op_project_fix(
    project_id: str,
    body: FixRequest,
    user: dict = Depends(get_current_operator),
):
    """Actually FIX the app — read the repo, have the AI edit the real code,
    commit it, and redeploy. This is what the Fix problem button calls so the
    AIs *act* instead of just explaining the error in chat.

    Write policy:
      * Platform repo (this app) → open a PR (never push to `main` directly).
      * Any other project → commit to `main` + trigger a redeploy so the live
        app is overwritten with the fix.
    """
    settings = await get_settings_doc()
    project = await db.deploy_projects.find_one({'id': project_id})
    if not project and project_id == SELF_PROJECT_ID:
    ...
    if not repo:
        raise HTTPException(
            ...
            {
                'error': 'no_repo',
                'message': 'This project has no linked GitHub repo yet, so there is no '
                           'code to fix. Push the app source first, then try again.',
                'initial_push_url': f'/api/operator/deploy/{project_id}/initial-push',
            },
        )
    ...
    from app_builder_ext import (
        GITHUB_API as _GH_API,
        _commit_files,
        _gh_get,
        _gh_post,
        _poll_deployment_ready,
        generate_ai_code_fix,
    )

    branch = project.get('gitRef') or 'main'
    fix = await generate_ai_code_fix(
        repo, branch, body.instruction, gh_token, provider=provider, model=model,
    )
    changed = fix['changed']
    if not changed:
        # The AI decided no code change was needed (or the fix is purely
        # infra). Surface its note instead of silently doing nothing.
        return {
            'ok': True,
            'mode': 'noop',
            'changed': [],
            'notes': fix['notes'] or 'The AI determined no code change was needed.',
            'redeployed': False,
        }
    ...
    async with httpx.AsyncClient(timeout=45.0) as client:
        if is_platform:
            ...
            ref_resp = await _gh_get(client, f'{_GH_API}/repos/{repo}/git/ref/heads/{branch}', gh_token)
            if ref_resp.status_code != 200:
                raise HTTPException(502, f'Could not read base ref `{branch}`: {ref_resp.text[:200]}')
            ...
            if mk.status_code not in (200, 201):
                raise HTTPException(502, f'Branch create failed: {mk.text[:200]}')
            committed = await _commit_files(client, gh_token, repo, new_branch, changed)
            pr_body = (
                f'**Automated fix**\n\n> {body.instruction[:1500]}\n\n'
                f'**AI summary:** {fix["notes"]}\n\n'
                f'**Files changed ({committed}):**\n'
                + '\n'.join(f'- `{p}`' for p in changed)
                + '\n\n_Generated by the Fix problem button · review before merging._'
            )
            ...
            if pr.status_code not in (200, 201):
                raise HTTPException(502, f'PR create failed: {pr.text[:300]}')
            pr_json = pr.json()
            return {
                'ok': True,
                'mode': 'pr',
                ...
                'branch': new_branch,
                'redeployed': False,
                'message': f'Opened a PR with the fix ({committed} file(s) changed). '
                           'Review and merge it to ship — direct pushes to the live '
                           'platform are blocked for safety.',
            }

        # User / chat app: overwrite in place + redeploy.
    ...
    try:
        res = await _trigger_deploy(
            project_id, settings, 'production', branch,
            bypass_review=True, user_id=user.get('sub'),
        )
        ...
        if deployment_id:
            ...
            state = (ready.get('readyState') or ready.get('status') or state or 'QUEUED').upper()
            if ready.get('url'):
                u = ready['url']
    ...
    except Exception as e:  # noqa: BLE001 — the code fix committed even if redeploy hiccups
        logger.warning('fix: redeploy after commit failed for %s: %s', project_id, str(e)[:200])
    ...
    return {
        ...
        'state': state,
        'redeployed': True,
        'message': f'Applied the fix to {committed} file(s) and redeployed your live app.',
    }
