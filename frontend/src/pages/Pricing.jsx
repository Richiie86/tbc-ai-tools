    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <section className="mx-auto max-w-7xl px-5 py-20">
        <div className="text-center">
          <div className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-400">Pricing</div>
        ...
        <div className="mt-14 grid gap-6 lg:grid-cols-3">
          {plans.map((p, idx) => {
            ...
            return (
              <div key={p.id} className={`relative rounded-2xl border p-7 ${featured ? 'border-amber-500/60 bg-slate-900/80 shadow-2xl shadow-amber-500/10 scale-[1.02]' : 'border-slate-800 bg-slate-900/50'}`}>
                {featured && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-amber-500 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-950">Most popular</div>
                )}
                <div className="flex items-center gap-3">
                  <div className={`grid h-11 w-11 place-items-center rounded-lg ${featured ? 'bg-amber-500 text-slate-950' : 'bg-amber-500/15 text-amber-300'}`}>
                ...
                {p.intro && p.regular_price && p.regular_price !== p.price ? (
                  <div className="mt-1 text-xs text-slate-400">First month only — then <span className="text-amber-300 font-semibold">${p.regular_price}/mo</span></div>
                ) : (
                  <div className="mt-1 text-xs text-slate-500">Billed monthly</div>
                )}
                <div className="mt-2 text-sm text-amber-300">{p.credits.toLocaleString()} AI messages included</div>
                ...
                <Button onClick={()=>checkout(p.id)} disabled={loadingPlan === p.id} className={`mt-8 w-full font-semibold ${featured ? 'bg-amber-500 text-slate-950 hover:bg-amber-400' : 'bg-slate-800 text-white hover:bg-slate-700 border border-slate-700'}`}>
